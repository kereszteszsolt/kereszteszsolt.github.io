# <img src="favicon.svg" width="40" height="40" alt=""> kereszteszsolt.net

**Looking for my personal website? Visit [kereszteszsolt.hu](https://kereszteszsolt.hu/).**

> kereszteszsolt.hu is available in Hungarian (HU), English (EN), Romanian (RO), and German (DE).

**kereszteszsolt.net** is the custom domain for [kereszteszsolt.github.io](https://kereszteszsolt.github.io/). This page and all the projects listed below are hosted on GitHub Pages.

[GitHub profile](https://github.com/kereszteszsolt) · [Source code for this page](https://github.com/kereszteszsolt/kereszteszsolt.github.io)

## 🎮 Projects

| Project | Description | Links |
| --- | --- | --- |
| **ClearField (Minesweeper)** | A mobile-friendly take on the classic Minesweeper puzzle. | [Play](https://kereszteszsolt.net/clear-field/) · [Source code](https://github.com/kereszteszsolt/clear-field) |
| **CrownGrid (Queens Game)** | Generate and play Queens-style logic puzzles. | [Play](https://kereszteszsolt.net/crown-grid/) · [Source code](https://github.com/kereszteszsolt/crown-grid) |

## 🍪 Privacy

This page adds no cookies, analytics, or other visitor trackers.

## ☕ Ways to support

**Explore ways to support the maintainer and their projects.**

[kereszteszsolt.hu/ways-to-support](https://kereszteszsolt.hu/ways-to-support/)

<p align="center">
  <a href="https://buymeacoffee.com/kereszteszsolt"><img src="docs/assets/buy-me-a-coffee-orange.png" alt="Buy Me a Coffee" width="360"></a><br>
  <strong>Every coffee counts! ☕❤️</strong>
</p>

---

<p align="center">
  <strong>Made with ❤️ by <a href="https://kereszteszsolt.hu/">Keresztes Zsolt</a></strong><br>
  ⭐ <a href="https://github.com/kereszteszsolt/kereszteszsolt.github.io">Star this repository</a> if you enjoy these projects.
</p>
